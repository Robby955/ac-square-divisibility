"""Verify source pins, independent tests and the optional pinned Lean build."""
...
def audit_sources():
    ...
    # This scan supplements kernel checking; comments are not proof evidence.
...
def main():
    ...
    if args.lean:
        ...
        commands.append(result)
        for name in ("ACSquareP2Checks", "ACSquareP3Checks", "PaperChecks"):
            result, text = run(["lake", "env", "lean", "--trust=0", f"{name}.lean"],
                               lean, logs / f"{name}-trust-zero.log")
            commands.append(result)
            found = re.findall(r"depends on axioms:\s*\[([^]]*)\]", text)
            expected = (lean / f"{name}.lean").read_text().count("#print axioms")
            # Lean can also report declarations with no axioms.
            ...
            reports += expected
    receipt = {"ok": True, "proof_source_modules": modules, "python_tests": 9,
